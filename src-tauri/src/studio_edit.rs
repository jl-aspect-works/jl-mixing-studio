...
fn read_edit_metadata(workspace: &Path) -> Result<(String, String), String> {
    ...
    let text = fs::read_to_string(&path)
        .map_err(|_| "Studio configuration could not be read.".to_owned())?;
    let document: Value = serde_json::from_str(&text)
        .map_err(|_| "Studio configuration is not valid JSON.".to_owned())?;
    let metadata = document
        .get("metadata")
        .and_then(Value::as_object)
        .ok_or_else(|| "Studio metadata is unavailable.".to_owned())?;
...
fn discovery_supports_update(home: &Path) -> Result<bool, String> {
    ...
    let supported = document
        ...
        .is_some_and(|items| {
            items
                .iter()
                .any(|item| item.as_str() == Some("studio.update"))
        });
    Ok(supported)
}
...
pub fn update_studio(app: &AppHandle, request: StudioUpdateRequest) -> StudioUpdateResult {
    ...
    match discovery_supports_update(&home) {
        ...
        Ok(false) => {
            return blocked(
                StudioUpdateCode::UnsupportedCapability,
                "The installed JL Mixing Automation does not support Studio editing.",
            );
        }
    ...
    match invoke_api(
        ...
        Ok(response) if response.status == ApiStatus::Success => {
            match read_edit_metadata(&workspace) {
                Ok((_, modified)) if modified != request.expected_last_modified_at => {
                    StudioUpdateResult {
                        ok: true,
                ...
                _ => blocked(
                    StudioUpdateCode::Uncertain,
                    "Automation reported success, but Studio could not verify the updated authoritative metadata. Refresh before retrying.",
                ),
            }
        }
        Ok(response) => {
            let message = response
                ...
                .unwrap_or_else(|| "JL Mixing Automation rejected the Studio update.".into());
            blocked(StudioUpdateCode::Rejected, message)
        }
        Err(ApiCallError::Unavailable) => blocked(
            StudioUpdateCode::AutomationUnavailable,
