...
pub(crate) fn log(level: &str, event: &str, fields: &[(&str, Value)]) {
    ...
    if level_rank(&normalized) < configured_rank() { return; }
    let path = log_path();
    ...
    for (key, value) in fields {
        ...
        let sensitive = ["password", "token", "secret", "credential", "authorization", "api_key"]
            .iter().any(|needle| lowered.contains(needle));
        record.insert((*key).into(), if sensitive { Value::from("<redacted>") } else { value.clone() });
    }
...
pub(crate) fn debug(event: &str, fields: &[(&str, Value)]) { log("debug", event, fields); }
pub(crate) fn info(event: &str, fields: &[(&str, Value)]) { log("info", event, fields); }
pub(crate) fn error(event: &str, fields: &[(&str, Value)]) { log("error", event, fields); }
