...
use commands::{
    ...
    execute_project_content_delete, get_comparison_results, get_comparison_setup,
    get_delivery_notes, get_jl_mixing_version, get_native_comparison_audio_status,
    get_native_project_audio_preview_status, get_project_audio_waveform, get_revision_notes,
    get_system_info, get_workspace_configuration, list_project_files,
    load_native_project_audio_preview, open_folder, open_project_file,
    pause_native_comparison_audio, pause_native_project_audio_preview, plan_project_content_delete,
    ...
    rename_revision_file, resolve_compact_audio_source, resolve_folder, reveal_project_file,
    ...
    set_workspace_root, stop_native_comparison_audio, stop_native_project_audio_preview,
...
fn get_original_delete_support(app: tauri::AppHandle) -> bool {
    resolve_home(&app)
        ...
        .is_some_and(|capabilities| {
            capabilities
                .iter()
                .any(|value| value == "client.files.delete.plan")
                && capabilities
                    .iter()
                    .any(|value| value == "client.files.delete.execute")
        })
}
...
#[tauri::command]
async fn execute_audio_prep_reset(
    app: tauri::AppHandle,
    request: AudioPrepResetRequest,
    progress: tauri::ipc::Channel<serde_json::Value>,
) -> Result<ManagedOperationResult, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let client_id = request.client_id.clone();
        let project_id = request.project_id.clone();
        managed_client_files::execute_reset_with_progress(&app, request, move |event| {
            let _ = progress.send(serde_json::json!({
                "clientId": &client_id,
                "projectId": &project_id,
                "phase": event.phase,
    ...
    .await
    .map_err(|error| format!("Audio Prep reset task failed: {error}"))
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            prepare_native_comparison_audio,
            play_native_comparison_audio,
            pause_native_comparison_audio,
            seek_native_comparison_audio,
            switch_native_comparison_candidate,
            set_native_comparison_audio_volume,
            stop_native_comparison_audio,
            get_native_comparison_audio_status,
