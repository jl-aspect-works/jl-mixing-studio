...
- supports Space for preview play/pause, comma for back five seconds, and period for forward five seconds; shortcuts remain inactive while typing in fields or using modified key combinations;
- accepts Enter from the preview controls or region form to save a region with valid start/end timestamps (end must follow start);
