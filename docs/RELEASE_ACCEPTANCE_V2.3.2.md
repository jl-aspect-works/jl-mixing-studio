...
**Qualification decision:** Studio RC1 had a Windows release blocker (#434) in per-region result saving. Its failed L03 remains the RC1 historical result. The save fix worked in a macOS development build per user report. Studio RC2 also changes the default to per-region matching (#436); verify L01–L03 and the other required cases on the installed RC2 package before stable `v2.3.2`. Automation RC1 remains the paired candidate. Do not dispatch stable until all required results and explicit approval are recorded.
