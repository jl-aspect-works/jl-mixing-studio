...
Studio `v2.3.2-rc.1` is a prerelease candidate that adds guarded Client Files deletion (#270) and optional per-region loudness matching (#396). It pairs with Automation `v2.3.2-rc.1` for managed Original Delivery deletion. Studio still uses Automation API `1.0` and workspace metadata schema `1.1.0`; compatibility is determined by advertised capabilities and supported schemas.
...
- **Per-region Loudness Match (#396):** Keep full-source matching as the default, or select **Each selected region**. Studio analyzes and caches each selected region using its precise boundaries; custom-only sessions are supported. Gain stays fixed throughout each region and changes when the region changes, without modifying source audio. The blind screen hides measurements; revealed results include per-region values.
...
- **Loudness Match:** Optional playback-only full-source matching uses the quietest candidate as the reference and attenuates louder candidates with fixed gain. The original audio is never changed.
