...
Reproduce a slow screen load or candidate switch and compare the phase durations in `studio.jsonl`. A started event without a terminal event can indicate an operation still pending or an interrupted application. Waveform requests superseded by a new selection are marked cancelled even though background decoding may finish later. Performance and waveform events contain no source paths, revision identities, notes, LUFS values, or raw error messages.

`comparison_playback` events are emitted only for meaningful playback transitions—not for the 100 ms status poll. They cover session readiness, play/pause, candidate switching, loop-boundary restart, natural end, retry, and status failures. Started and terminal events share an operation ID. For intermittent acceptance troubleshooting these events intentionally include the active and target revision ID, revision number, and blind letter, together with provider state, loop state, Full Song/custom-region classification, and millisecond playback position. They do not include source paths or raw error messages.
