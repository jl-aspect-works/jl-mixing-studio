...
const projectScreenLabel: Partial<Record<ProjectShellView, string>> = {
  intake: "Client Files",
...
export default function StudioApp() {
  ...
  const projectDeleted = () => { setSelectedProject(null); setSelectedClientId(null); setProjectView("overview"); setActiveRoute("projects"); intake.clear(); revision.reset(); approval.reset(); delivery.setState({ status: "closed" }); setRouteNotice("Project deleted permanently. External Listening copies were retained."); void resources.refreshWorkspace(false); };
  const clientDeleted = () => { setSelectedClientId(null); setSelectedProject(null); setProjectView("overview"); setActiveRoute("clients"); clients.setState({ status: "closed" }); projects.setState({ status: "closed" }); intake.clear(); revision.reset(); approval.reset(); delivery.setState({ status: "closed" }); setClientNotice(null); setProjectNotice(null); setRouteNotice("Client deleted permanently. Recovery is not available."); void resources.refreshWorkspace(); };
  ...
  return <div className={`app-shell${preferences.compactLayout ? " compact-layout" : ""}${preferences.reduceMotion ? " reduce-motion" : ""}`}>
    ...
    <main className={`main-content compact-global-headers${showOverviewToolbar ? " project-overview-open" : ""}`} id="main-content">
      ...
      {projectOpen && projectView !== "intake" && intake.progress && <ValidationProgress progress={intake.progress} />}
