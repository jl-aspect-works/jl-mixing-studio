...
import { formatOverviewDate, formatOverviewDateTime, getIntakeOverviewStatus, getProjectLastModified } from "./ProjectOverviewModel";
...
export function ProjectOverviewSummary({ project, tasks, intakeReport }: { project: ProjectSummary; tasks: DerivedTask[]; intakeReport: IntakeReportState }) {
  ...
  const metrics = [
    ...
    { label: "Current Revision", value: revisionLabel(project.currentRevision), detail: formatOverviewDateTime(project.revisions.find((revision) => revision.number === project.currentRevision)?.createdAt) },
