...
export function AudioPreviewPlayer({
  ...
  return <div className={`shared-audio-preview-inline${standardTransport ? " standard-transport" : ""}`} aria-label={`Preview ${entry.displayName}`} aria-busy={loading}>
    ...
    />
    {standardTransport && <button type="button" className="shared-audio-preview-skip icon-only" aria-label="Back 5 seconds" title="Back 5 seconds" disabled={!duration} onClick={() => seek(Math.max(0, currentTime - 5))}><ActionIcon name="skipBack" /></button>}
    ...
    {standardTransport && <button type="button" className="shared-audio-preview-skip icon-only" aria-label="Forward 5 seconds" title="Forward 5 seconds" disabled={!duration} onClick={() => seek(Math.min(duration, currentTime + 5))}><ActionIcon name="skipForward" /></button>}
