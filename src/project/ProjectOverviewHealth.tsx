...
import { getAudioValidationOverviewStatus, getDeliveryOverviewStatus, getRevisionOverviewStatus, getTaskOverviewStatus, type OverviewStatus } from "./ProjectOverviewModel";
...
const getClientFilesStatus = (fileIndex: ProjectOverviewFileIndex): OverviewStatus => {
  if (fileIndex.status === "loading") return { label: "Checking", detail: "Reading Client Files", tone: "neutral" };
  if (overviewAreaHasFailure(fileIndex, projectFilePaths.originalDelivery)) {
    return { label: "Unavailable", detail: "Client Files could not be indexed", tone: "neutral" };
  }
  const count = fileIndex.folders.clientFiles.fileCount;
  if (count === 0) return { label: "Empty", detail: "No client files", tone: "neutral" };
  return { label: "Available", detail: `${count} client ${count === 1 ? "file" : "files"}`, tone: "good" };
};
...
export function ProjectOverviewHealth({ project, tasks, intakeReport, fileIndex }: { project: ProjectSummary; tasks: DerivedTask[]; intakeReport: IntakeReportState; fileIndex: ProjectOverviewFileIndex }) {
  const clientFilesStatus = getClientFilesStatus(fileIndex);
  const audioValidationStatus = getAudioValidationOverviewStatus(intakeReport);
  ...
  const audioPrepStatus = getAudioPrepStatus(fileIndex, audioValidationStatus);
  const overallAttention = audioValidationStatus.tone === "attention" || taskStatus.tone === "attention";
  ...
  return (
    <section className="overview-card overview-health-card" aria-labelledby="overview-health-heading">
      ...
      <div className="overview-health-list">
        <HealthRow label="Client Files" status={clientFilesStatus} />
