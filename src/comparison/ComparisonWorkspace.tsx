...
export function ComparisonWorkspace({
  ...
  const playbackEnd = playback ? Math.min(region.endSeconds ?? playback.durationSeconds, playback.durationSeconds) : region.endSeconds ?? region.startSeconds;
  ...
  return <section className="comparison-workspace" aria-labelledby="comparison-workspace-title">
    ...
    <section className="panel comparison-listening" aria-labelledby="comparison-listening-title">
      ...
      <div className="comparison-seek-shell"><span>{formatTimestamp(playback?.currentSeconds ?? region.startSeconds)}</span><input type="range" min={region.startSeconds} max={playbackEnd} step="0.05" value={playback?.currentSeconds ?? region.startSeconds} aria-label="Comparison playback position" disabled={!playback || playbackBusy || !!playbackError} onChange={(event) => void seekPlayback(Number(event.target.value))} /><span>{formatTimestamp(playbackEnd)}</span></div>
      {playback && region.endSeconds !== null && playback.durationSeconds < region.endSeconds && <small>Candidate {activeCandidate} ends before this region’s defined end; playback and looping use its actual end.</small>}
