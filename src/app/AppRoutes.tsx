...
export function AppRoutes(p: AppRoutesProps) {
  ...
  return <AppProjectSection workspace={p.workspace} selected={p.selectedProject} client={p.route.resolvedProjectClient} project={p.route.resolvedProject} view={p.projectView} loading={p.loading} intakeReport={p.intake.reportState} intakeActionError={p.intake.actionError} intakeValidationAvailable={p.availability.intakeValidationAvailable} intakeValidationHelp={p.availability.intakeValidationHelp} intakeLoading={intakeLoading} intakeProgress={p.intake.progress} revisionActionError={p.revision.actionError ?? p.approval.actionError} revisionCreationAvailable={p.availability.revisionCreationAvailable} revisionCreationHelp={p.availability.revisionCreationHelp} revisionApprovalAvailable={p.availability.revisionApprovalAvailable} revisionApprovalHelp={p.availability.revisionApprovalHelp} deliveryActionError={p.delivery.actionError} deliveryCreationAvailable={p.route.deliveryCreationAvailable} deliveryCreationHelp={p.route.deliveryCreationHelp} deliveryLoading={p.loading || p.delivery.state.status === "preflighting" || p.delivery.state.status === "creating"} projectCreationAvailable={p.availability.projectCreationAvailable} projectCreationHelp={p.availability.projectCreationHelp} onProjects={p.onProjects} onRefresh={p.onRefresh} onProjectSaveSuccess={p.projects.setNotice} onIntakeRefresh={() => { p.onRefresh(); p.intake.reload(); }} onStructuredValidationRefresh={p.intake.refreshStructured} onSelectView={p.onSelectProjectView} onOpenIntake={p.intake.open} onRecheckIntake={p.intake.recheck} onOpenRevisions={p.onOpenRevisions} onNewRevision={p.revision.open} onApproveRevision={p.approval.open} onCreateDelivery={p.delivery.open} onNewProject={() => p.projects.open(null, false)} onSelectProject={p.onOpenDerivedProject} />;
}
