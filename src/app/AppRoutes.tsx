...
import { ClientDetails, ClientsRoute } from "../AppViews";
import { ActivityRoute, TasksRoute } from "../activity/TaskActivityRoutes";
...
export interface AppRoutesProps {
  ...
  onNewClient: () => void;
  onClientSaveSuccess: (message: string | null) => void;
...
export function AppRoutes(p: AppRoutesProps) {
  ...
  if (p.activeRoute === "clients") return p.route.resolvedClient ? <ClientDetails client={p.route.resolvedClient} tasks={p.workspace.status === "ready" ? p.workspace.value.tasks : []} onBack={() => p.onSelectClient(null)} onRefresh={p.onRefresh} onSaveSuccess={p.onClientSaveSuccess} onDeleted={p.onClientDeleted} loading={p.loading} onNewProject={() => p.projects.open(p.route.resolvedClient!.clientId, true)} projectCreationAvailable={p.availability.projectCreationAvailable} projectCreationHelp={p.availability.projectCreationHelp} onSelectProject={(projectId) => p.onOpenClientProject(p.route.resolvedClient!.clientId, projectId)} /> : <ClientsRoute workspace={p.workspace} onSelectClient={p.onSelectClient} onNewClient={p.onNewClient} onRefresh={p.onRefresh} loading={p.loading} clientCreationAvailable={p.availability.clientCreationAvailable} clientCreationHelp={p.availability.clientCreationHelp} />;
  if (p.activeRoute !== "projects") return null;
  const intakeLoading = p.loading || p.intake.state.status === "preflighting" || p.intake.state.status === "running";
  return <AppProjectSection workspace={p.workspace} selected={p.selectedProject} client={p.route.resolvedProjectClient} project={p.route.resolvedProject} view={p.projectView} loading={p.loading} intakeReport={p.intake.reportState} intakeActionError={p.intake.actionError} intakeValidationAvailable={p.availability.intakeValidationAvailable} intakeValidationHelp={p.availability.intakeValidationHelp} intakeLoading={intakeLoading} intakeProgress={p.intake.progress} revisionActionError={p.revision.actionError ?? p.approval.actionError} revisionCreationAvailable={p.availability.revisionCreationAvailable} revisionCreationHelp={p.availability.revisionCreationHelp} revisionApprovalAvailable={p.availability.revisionApprovalAvailable} revisionApprovalHelp={p.availability.revisionApprovalHelp} deliveryActionError={p.delivery.actionError} deliveryCreationAvailable={p.route.deliveryCreationAvailable} deliveryCreationHelp={p.route.deliveryCreationHelp} deliveryLoading={p.loading || p.delivery.state.status === "preflighting" || p.delivery.state.status === "creating"} projectCreationAvailable={p.availability.projectCreationAvailable} projectCreationHelp={p.availability.projectCreationHelp} onProjects={p.onProjects} onRefresh={p.onRefresh} onProjectDeleted={p.onProjectDeleted} onProjectSaveSuccess={p.projects.setNotice} onIntakeRefresh={() => { p.onRefresh(); p.intake.reload(); }} onStructuredValidationRefresh={p.intake.refreshStructured} onSelectView={p.onSelectProjectView} onOpenIntake={p.intake.open} onRecheckIntake={p.intake.recheck} onOpenRevisions={p.onOpenRevisions} onNewRevision={p.revision.open} onApproveRevision={p.approval.open} onCreateDelivery={p.delivery.open} onNewProject={() => p.projects.open(null, false)} onSelectProject={p.onOpenDerivedProject} />;
}
