...
export function AppDialogs({ workspace, project, studio, clients, projects, intake, revision, approval, delivery, onRefresh }: AppDialogsProps) {
  return <>
    ...
    {delivery.state.status === "options" && project?.approvedRevision !== null && project?.approvedRevision !== undefined && <DeliveryOptionsDialog
      ...
      deliveryNoteLoading={delivery.deliveryNoteLoading}
      deliveryNoteError={delivery.deliveryNoteError}
      deliveryNoteMaxBytes={delivery.deliveryNoteMaxBytes}
      onCleanFirstChange={delivery.setCleanFirst}
      onDeliveryNoteChange={delivery.setDeliveryNoteDraft}
