...
describe("JL Mixing Studio — workspace and studio states", () => {
  ...
  it("shows setup guidance for an unavailable default workspace", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Create the default studio workspace" })).toBeInTheDocument();
  ...
  it("distinguishes a valid empty workspace", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "No recent project yet" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Clients" }));
    expect(await screen.findByRole("button", { name: "New client" })).toBeEnabled();
  });
  ...
  it("blocks project presentation when studio configuration is invalid", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Workspace unavailable" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Studio" }));
    expect(await screen.findByRole("heading", { name: "Studio configuration is not readable" })).toBeInTheDocument();
  ...
  it("keeps the read-only dashboard usable for an unsupported automation version", async () => {
    ...
    expect((await screen.findAllByText(/guided creation requires 1.3.1/i)).length).toBeGreaterThan(0);
    fireEvent.click(screen.getByRole("button", { name: "Clients" }));
    expect(screen.getByRole("button", { name: "New client" })).toBeDisabled();
  });
});
