...
export function ManagedFileOperationDialog({ clientId, projectId, mode, relativePaths = [], title, sourceCancelLabel = "Cancel", onClose, onCompleted }: ManagedFileOperationDialogProps) {
  ...
  return <div className="dialog-backdrop" onKeyDown={(event) => { if (event.key === "Escape" && !pending) onClose(); }}>
    <section className="client-dialog managed-file-dialog" role="dialog" aria-modal="true" aria-labelledby="managed-file-dialog-title">
      ...
      {review && <><p className="dialog-intro">Review what Studio will change before continuing.</p><section className="managed-plan-summary" aria-label="Managed file operation summary">{mode === "import" && <article><strong>{originalItems.length}</strong><span>Original Delivery operations</span></article>}<article><strong>{audioItems.length}</strong><span>Audio Prep operations</span></article><article><strong>{conflicts.length}</strong><span>Need a decision</span></article></section>
        {conflicts.length === 0 ? <p className="managed-no-conflicts">No existing project files will be overwritten.</p> : <section className="managed-conflicts" aria-labelledby="managed-conflicts-heading"><div className="managed-conflicts-heading"><div><h3 id="managed-conflicts-heading">{conflicts.length} {conflicts.length === 1 ? "file needs" : "files need"} your attention</h3><p>Every existing destination requires an explicit Replace or Skip decision.</p></div>{conflicts.length > 1 && <div className="managed-apply-all"><span>Apply to all:</span><button type="button" className="secondary" onClick={() => setAll("replace")} disabled={pending}>Replace All</button><button type="button" className="secondary" onClick={() => setAll("skip")} disabled={pending}>Skip All</button></div>}</div><div className="managed-conflict-list">{conflicts.map((item) => <article key={item.id} className={item.area === "audio_prep" ? "audio-prep-conflict" : ""}><div><strong>{fileLabel(item)}</strong><span>{areaLabel(item.area)}</span>{item.area === "audio_prep" && <small>Replacing this file will overwrite the current prepared version.</small>}</div><div className="managed-conflict-actions" role="group" aria-label={`Decision for ${fileLabel(item)} in ${areaLabel(item.area)}`}><button type="button" className={decisions[item.id] === "replace" ? "selected" : "secondary"} aria-pressed={decisions[item.id] === "replace"} onClick={() => setDecisions((current) => ({ ...current, [item.id]: "replace" }))} disabled={pending}>Replace</button><button type="button" className={decisions[item.id] === "skip" ? "selected" : "secondary"} aria-pressed={decisions[item.id] === "skip"} onClick={() => setDecisions((current) => ({ ...current, [item.id]: "skip" }))} disabled={pending}>Skip</button></div></article>)}</div></section>}
