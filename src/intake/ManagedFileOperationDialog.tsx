...
const groupedRows = (plan: ManagedPlan): ReviewRow[] => {
  ...
  for (const file of plan.files) rows.set(file.relative_path, { sourcePath: file.relative_path, original: null, audio: null });
...
export function ManagedFileOperationDialog({
  ...
  const setAll = (decision: ManagedConflictDecision) => setDecisions((current) => ({ ...current, ...Object.fromEntries(activeConflicts.map((item) => [item.id, decision])) }));
  ...
  const actionCell = (item: ManagedPlanItem | null, area: "original" | "audio") => {
    ...
    return <div className="managed-action-cell">
      ...
      <select aria-label={`${area === "original" ? "Client Files" : "Audio Prep"} action for ${item.source_relative_path}`} value={selected} onChange={(event) => setDecision(item, event.target.value)}>
        <option value="">Choose…</option><option value="replace">Replace</option><option value="skip">Skip</option>
      </select>
  ...
  const execute = async () => {
    ...
    const activeDecisions = mode === "import"
      ...
      : decisions;
    const executingState: DialogState = { status: "executing", plan: review.plan, sourceKind: review.sourceKind, sources: review.sources };
  ...
  const importTotal = state.status === "executing" ? state.plan.files.length : 0;
  const displayImportTotal = importProgress?.total ?? importTotal;
  ...
  return <div className="dialog-backdrop" onKeyDown={(event) => { if (event.key === "Escape" && !pending) onClose(); }}>
    <section className="client-dialog managed-file-dialog" role="dialog" aria-modal="true" aria-labelledby="managed-file-dialog-title">
      ...
      {state.status === "executing" && <div className="managed-operation-progress managed-operation-progress-primary" role="status" aria-live="polite">
        ...
        {mode === "import" && importProgress?.active.length ? <small>Processing: {importProgress.active.map((path) => path.split(/[\\/]/).pop() ?? path).join(" · ")}</small> : null}
        {mode === "import" && !importProgress && <p>Starting import…</p>}
      </div>}
